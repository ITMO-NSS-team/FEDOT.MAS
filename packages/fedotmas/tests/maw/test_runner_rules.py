...
class TestSearchLimitDoesNotRestartPipeline:
    """A limit exception is no longer converted into a whole-MAS recovery turn."""

    @pytest.mark.asyncio
    async def test_limit_exception_is_not_retried_as_a_second_pipeline_turn(
        self, mock_session_service
    ):
        calls = []

        async def failing_run_async(**_kwargs):
            calls.append(True)
            raise WebSearchLimitExceeded("limit hit")
            yield  # pragma: no cover

        runner_instance = MagicMock()
        runner_instance.run_async = failing_run_async
