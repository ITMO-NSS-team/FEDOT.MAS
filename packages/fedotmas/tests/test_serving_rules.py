...
from fedotmas.serving import MASAgentLoader, create_api_app
...
class TestCreateApiApp:
    ...
    def test_custom_session_uri(self, mock_get):
        create_api_app({}, session_service_uri="sqlite:///s.db")
        _, kwargs = mock_get.call_args
    ...
    def test_empty_agents_dict(self, mock_get):
        ...
        assert isinstance(loader, MASAgentLoader)
        assert loader.list_agents() == []
    ...
    def test_agents_registered_in_loader(self, mock_get):
        ...
        create_api_app({"a": agent_a, "b": agent_b})
        _, kwargs = mock_get.call_args
        loader: MASAgentLoader = kwargs["agent_loader"]
        assert isinstance(loader, MASAgentLoader)
