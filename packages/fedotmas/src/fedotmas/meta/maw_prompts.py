...
- For long-horizon investigation, use `research-controller` when it is available to the agent. Use a stable, distinct `research_id` for the workstream; periodically update its state with new queries, sources, evidence, unresolved questions, failed approaches, confidence, and remaining budget. Call `get_next_action` before continuing expensive searches and follow its recommendation to continue, change strategy, or synthesize. The controller tracks state and recommends actions; it does not gather evidence.
