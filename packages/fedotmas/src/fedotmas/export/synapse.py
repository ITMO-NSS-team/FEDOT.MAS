"""Emit a ``MAWConfig`` or ``MASConfig`` as a Synapse configuration bundle.
...
class _Walk:
    ...
    ids: dict[str, MAWAgentConfig | MASAgentConfig] = field(default_factory=dict)
...
def to_synapse_bundle(
    ...
    *,
    ...
    unmatched = tuple(sorted(external.keys() - {a.name for a in source_agents}))
    if unmatched:
        _log.warning(
    ...
    workflow = {
        ...
        "description": f"Generated by FEDOT.MAS from {len(source_agents)} agents",
...
def _emit_mas(
    ...
    coordinator["system_prompt"] += (
        ...
        "Call delegate_to_agent with agent_id from the roster below and a non-empty "
        "instruction containing the subtask, original request constraints and any "
        "relevant previous results. Original worker names refer to these agent_ids, "
        "not separate tools. Delegation returns control and the child result to you. "
        "Check status and child_status; handle failures before synthesizing the final "
        "answer. Tool results are not automatically written to SharedContext.\n"
        + json.dumps(roster, ensure_ascii=False)
    )
    if config.coordinator.output_key:
        coordinator["system_prompt"] += (
            "\nWrite the final synthesized value with context_write using key "
            + json.dumps(config.coordinator.output_key, ensure_ascii=False)
            + " before completing the task."
        )
    node_id = to_wire_name(config.coordinator.name, walk.used)
    node: dict[str, Any] = {
        "id": node_id,
        "type": "phase",
    ...
    if config.coordinator.output_key:
        node["writes"] = [config.coordinator.output_key]
    walk.nodes.append(node)
    return node_id, node_id


def _agent_doc(
    agent: MAWAgentConfig | MASAgentConfig,
    wire_id: str,
    *,
    reused: bool,
    temperature: float,
...
def _last_agent(
    step: MAWStepConfig, walk: _Walk
) -> MAWAgentConfig | MASAgentConfig | None:
