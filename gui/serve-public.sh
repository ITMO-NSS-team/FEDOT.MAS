...
# Провайдер туннеля. По умолчанию localhost.run: serveo на бесплатном тарифе рвёт
# любой запрос длиннее ~6 секунд (502), а у нас разбор постановки идёт 15 секунд,
# ответ одной модели и судья — до минуты. Проверено замером, не предположение.
# pinggy по умолчанию: адрес держится всю сессию. У localhost.run адрес меняется
