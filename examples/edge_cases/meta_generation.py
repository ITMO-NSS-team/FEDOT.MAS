...
async def two_stage_simple():
    ...
    try:
        mas = MAS(two_stage=True, meta_model=META_MODEL)
        config = await mas.generate_config("Explain what a REST API is")
        _log.info("Generated config: {} agents, pipeline_type={}", len(config.agents), config.pipeline.type)
        state = await mas.build_and_run(config, "Explain what a REST API is")
...
async def single_stage_simple():
    ...
    try:
        mas = MAS(two_stage=False, meta_model=META_MODEL)
        config = await mas.generate_config("List 3 benefits of exercise")
        _log.info("Generated config: {} agents, pipeline_type={}", len(config.agents), config.pipeline.type)
        state = await mas.build_and_run(config, "List 3 benefits of exercise")
        _log.info("Result state keys: {}", list(state.keys()))
        for key in state:
            if key != "user_query":
                _log.info("Output [{}]: {}", key, str(state[key])[:200])
...
async def minimal_task():
    ...
    try:
        mas = MAS(two_stage=True, meta_model=META_MODEL)
        config = await mas.generate_config("Summarize AI")
        _log.info("Generated config: {} agents, pipeline_type={}", len(config.agents), config.pipeline.type)
        state = await mas.build_and_run(config, "Summarize AI")
        _log.info("Result state keys: {}", list(state.keys()))
        for key in state:
            if key != "user_query":
                _log.info("Output [{}]: {}", key, str(state[key])[:200])
