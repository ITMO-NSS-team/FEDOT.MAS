...
`run.py` reads the initial pipeline from `benchmarks/aime_math/seed_config.json`.
Default files (generated by the FEDOT.MAS meta-agent for `gpt-4.1-mini` and
`gpt-oss-120b`) are committed to the repo, so the benchmark works out of the
box. To regenerate the seed (e.g. with a different solver model or task
description), run:
