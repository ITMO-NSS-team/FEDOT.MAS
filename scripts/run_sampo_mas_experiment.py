...
TASK = """Map historical construction work names to one of the provided canonical granular work labels. Design and execute an efficient workflow. The production dataset contains millions of rows, so avoid unnecessary per-row LLM calls and exploit batching/deduplication/code where useful. Use only the public sampo-benchmark tools and safe code tools. Do not attempt to access databases, mapping tables, labels not supplied by the tool, or private ground truth. For this run, call run_tfidf_baseline with max_examples={count} and output_filename='{filename}', then report the prediction path."""
...
async def run() -> None:
    ...
    config = await mas.generate_config(
        TASK.format(count=1000, filename="pilot_predictions.csv")
    )
    ...
    pilot_state = await mas.build_and_run(
        config, TASK.format(count=1000, filename="pilot_predictions.csv"), timeout=900
    )
    full_state = await mas.build_and_run(
        config, TASK.format(count=15039, filename="full_predictions.csv"), timeout=1800
    )
    predictions_dir = ROOT / "artifacts" / "sampo_benchmark" / "mas_runs"
    for source_name, destination_name in (
        ("pilot_predictions.csv", "pilot_predictions.csv"),
        ("full_predictions.csv", "predictions.csv"),
    ):
    ...
    (OUT / "full_state.json").write_text(
        json.dumps(full_state, default=str, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
